...
module.exports = function hydratePdf(path, document, changes, next) {
  outfolder = '/tmp/' + crypto.randomBytes(20).toString('hex') + '/';
  ...
  async.waterfall([
    ...
    function(cb) {
      pdfOcrChild = shellExec('convert ' + path + ' ' + outfolder + '%04d' + '.png', function(err, stdout, stderr) {
        pdfOcrChild = null;
        if(err) {
...
process.on('exit', function() {
  if(pdfOcrChild) {
    console.log('Emergency SIGKILL of pdf converter:' + pdfOcrChild.pid);
    execSync('kill -s SIGKILL ' + pdfOcrChild.pid);
    try {
